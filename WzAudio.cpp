...
WzAudio::WzAudio(std::shared_ptr<WzReader> r, uint32_t o,uint32_t len, int32_t header) :reader(r),offset(o), length(len), header(header)
{
}

auto WzAudio::getId() ->size_t
{
	return offset;
}

auto WzAudio::getLength() ->uint32_t
{
	return length;
}
...
auto WzAudio::isPcm() -> bool
{
	return 0x12 == header;
}

auto WzAudio::getChannel() -> int32_t
{
	return 1;
}

auto WzAudio::getRate() -> int32_t
{
	return rate;
}

auto WzAudio::getBits() -> int32_t
{
	return bits;
}

auto WzAudio::getWaveHeader() -> byte *
{
	reader->setPosition(offset - header);
	if (1 == reader->readShort())
	{
		reader->setPosition(offset - header);
		return reader->readBytes(0x10);
	}
	reader->setPosition(offset - header);
	return reader->decryptBytes(reader->readBytes(0x10), 0x10);
}
...
auto WzAudio::getData() -> std::shared_ptr<std::vector<uint8_t>>&
{
	...
	data->insert(data->begin(), dataPtr, dataPtr + length);
	delete[] dataPtr;
	return data;
}

auto WzAudio::getAudioData() -> byte *
{
	reader->setPosition(offset);
